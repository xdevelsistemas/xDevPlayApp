...
object Build extends sbt.Build with MoreTasks {
  ...
  val controllersDeps = essencialDeps ++ Seq(
  ...
  lazy val controllers = play.Project(
    appName + "-controllers", appVersion,
    controllersDeps,
    path = file("modules/controllers"),
    settings = BuildSettings.buildSettings
  ).settings(
      zap <<= zapTask,
      zap <<= zap.dependsOn(clean in Compile)
  ...
  lazy val root = play.Project(
    ...
    .aggregate(models,services,controllers)
    .dependsOn(models,services,controllers)
