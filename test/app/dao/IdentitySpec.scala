...
import dao.DAO.{userDAO,identityDAO}


class IdentitySpec extends PlaySpecification with Mockito {
  ...
  "IdentityDAO" should {
    "be able to find Identity given its IdentityId" in {
      new WithApplication(FakeApplication()) {
        ...
        val u = userDAO.create(mockIdentity)
        val i = identityDAO.create(u, mockIdentity)
